...
# SiPe.Sat, a Risat UKSW harvester program
# Copyright (C) 2023 Samarthya Lykamanuella (lykamanuella@outlook.com)
# 
...
# The about screen, displaying short description about SiPe.Sat
class SipesatScrAbout(tk.Frame):
    
    # The content of the message screen
    content = '''
SIPESAT (Sistem Pemanen Data Risat) adalah program turunan
dari situs web Risat (https://risat.uksw.edu) berbasis desktop.
SIPESAT memiliki fungsi utama untuk memanen data penelitian
dan pengabdian masyarakat berjumlah besar dalam waktu
singkat, kemudian menyimpannya ke dalam suatu file spreadsheet.

Program ini utamanya ditujukan kepada para administrator Risat.
Adanya program SIPESAT mempersingkat waktu administrator Risat
dalam meninjau proposal dan laporan penelitian maupun pengabdian
masyarakat di situs web Risat. Sehingga, proses peninjauan dapat
dilakukan dengan lebih mudah. SIPESAT ditulis dalam bahasa
pemrograman Python dan dibangun di atas framework TkInter.
        
Dibuat oleh Samarthya Lykamanuella @ 2023
...
'''
    ...
    def __init__(self, parent, controller):
        ...
        # Back button
        back_text = 'KEMBALI'
        back_button = ttk.Button(self, text=back_text, width=30,
            command=lambda: self.on_back_button_click())
        back_button.grid(row=4, column=0, padx=5, pady=5)

# The license screen, displaying the license notice of SiPe.Sat
class SipesatScrLicense(tk.Frame):
...
class SipesatScrComService(tk.Frame):
    ...
    def on_next_button_click(self):
        ...
        match l:
            ...
            case 'Risat Telah Direview Pengabdian Masyarakat':
                m = 'teldireview'
            case 'Risat Disetujui Pengabdian Masyarakat':
                m = 'disetujui'
    ...
    def __init__(self, parent, controller):
        ...
        harvesttype_options = [
            ...
            'Risat Disetujui Pengabdian Masyarakat',
...
class BackEndHarvester():
        ...
        # ---------------- RISAT PENGABDIAN MASYARAKAT ---------------- #
            ...
            if datatype == 0: # --- 'data ringkasan'
                ...
                match output:
                    ...
                    case 'disetujui':
                        self.run_harvest_c_0_disetujui(control, username, password)
                    case 'ddr':
                ...
                match output:
                    ...
                    case 'disetujui':
                        self.run_harvest_c_1_disetujui(control, username, password)
                    case 'ddr':
    ...
    # This function opens "Disetujui Pengabdian" menu after opening the tab "Pengabdian"
    # - Requires 'data_prompt' array as an unary argument obtained from get_risat_pengabdian() function
    # - Returns also another 'data_prompt' array
    def get_risat_pengabdian_disetujui_pengabdian(self, data_prompt):
        # Logging the calling of the function
        print('+ Opening "Pengabdian --> Disetujui Pengabdian" menu...')
    ...
    # This function opens the detail page of "Disetujui Pengabdian" entry row
    # Requires one argument:
    # - the data prompt value array in compliance with convention [5] of this file
    # The 'data_prompt' argument passed into this function must be
    # rooted from the function 'get_risat_pengabdian_disetujui_pengabdian()'
    def get_risat_pengabdian_disetujui_pengabdian_detil(self, data_prompt):
        # Logging the calling of the function
        print('+ Opening "Disetujui Pengabdian" entry row detail page...')
    ...
    # This function harvests "Risat Disetujui Pengabdian > Ringkasan Data" data
    # and then store the harvested data as an excel file
    #
    # Required arguments:
    # - control             --> for updating the progress bar and
    #                           message area of the screen SipesatScrHarvest
    # - username, password  --> the Risat administrator username and password
    def run_harvest_c_0_disetujui(self, control, username, password):
        # SipesatScrHarvest messenger
        control.set_header_desc('Panen Data "Risat Disetujui Pengabdian > Ringkasan Data"')
        control.set_help_label('Data sedang dipanen. Silahkan menunggu.')
        control.set_progress_bar(0)
        control.clear_message_area()
        ...
        data_prompt = self.get_risat_login(username, password)
        data_prompt = self.get_risat_pengabdian(data_prompt)
        data_prompt = self.get_risat_pengabdian_disetujui_pengabdian(data_prompt)
        ...
        workbook = xl.Workbook()
        sheet = workbook.active
        sheet.title = 'Disetujui Pengabdian Ringkasan'
        ...
        while True:
            ...
            output_spreadsheet = filedialog.asksaveasfilename(
                filetypes=[('Excel files', '*.xlsx')],
                initialfile='Sipesat - Disetujui Pengabdian Ringkasan Risat.xlsx',
                title='Simpan sebagai ...'
            )
    ...
    # This function harvests "Risat Disetujui Pengabdian > Data Detil Lengkap" data
    # and then store the harvested data as an excel file
    #
    # Required arguments:
    # - control             --> for updating the progress bar and
    #                           message area of the screen SipesatScrHarvest
    # - username, password  --> the Risat administrator username and password
    def run_harvest_c_1_disetujui(self, control, username, password):

        # SipesatScrHarvest messenger
        control.set_header_desc('Panen Data "Risat Disetujui Pengabdian > Data Detil Lengkap"')
        control.set_help_label('Data sedang dipanen. Silahkan menunggu.')
        control.set_progress_bar(0)
        control.clear_message_area()
        ...
        workbook = xl.Workbook()
        sheet = workbook.active
        sheet.title = 'Disetujui Pengabdian Detil'
        ...
        for i in range(number_of_row):
            ...
            # Obtaining the response data of each individual entry row detail page
            data = self.get_risat_pengabdian_disetujui_pengabdian_detil(detail_prompt)
            content = data['html_content']
            response = data['http_response']
            ...
            # Use try-except catching to mitigate empty data
            try:
                h2_pre = content.xpath('//div[@id="ContentPlaceHolder1_daftar2_kiri1_updx"]/div[1][@class="panel panel-info"]/div[@class="panel-body f12"]/div[5][@class="row f12"]/div[@class="col-sm-4"]//text()')
                for l in h2_pre:
                    l = l.replace('\r','').replace('\n','').strip()
                    h2 += l +', '
                # Remove trailing ', ' characters
            ...
            # Use try-except catching to mitigate empty data
            try:
                ae2 = content.xpath('//div[@id="ContentPlaceHolder1_daftar2_kiri1_updx"]/div[3][@class="panel panel-info"]/div[@class="panel-body f12"]/div[1][@class="row"]/div[@class="col-sm-4"]/text()')[0].replace('\r','').replace('\n','').strip()
            except IndexError:
                ae2 = ''
            # Use try-except catching to mitigate empty data
            try:
                af2 = content.xpath('//div[@id="ContentPlaceHolder1_daftar2_kiri1_updx"]/div[3][@class="panel panel-info"]/div[@class="panel-body f12"]/div[2][@class="row"]/div[@class="col-sm-4"]/text()')[0].replace('\r','').replace('\n','').strip()
            except IndexError:
                af2 = ''
            # Use try-except catching to mitigate empty data
            try:
                ag2 = content.xpath('//div[@id="ContentPlaceHolder1_daftar2_kiri1_updx"]/div[3][@class="panel panel-info"]/div[@class="panel-body f12"]/div[3][@class="row"]/div[@class="col-sm-4"]/text()')[0].replace('\r','').replace('\n','').strip()
            except IndexError:
                ag2 = ''
            # Use try-except catching to mitigate empty data
            try:
                ah2 = content.xpath('//div[@id="ContentPlaceHolder1_daftar2_kiri1_updx"]/div[3][@class="panel panel-info"]/div[@class="panel-body f12"]/div[4][@class="row"]/div[@class="col-sm-4"]/text()')[0].replace('\r','').replace('\n','').strip()
            except IndexError:
                ah2 = ''
            # Use try-except catching to mitigate empty data
            try:
                ai2 = content.xpath('//div[@id="ContentPlaceHolder1_daftar2_kiri1_updx"]/div[3][@class="panel panel-info"]/div[@class="panel-body f12"]/div[5][@class="row"]/div[@class="col-sm-4"]/text()')[0].replace('\r','').replace('\n','').strip()
            except IndexError:
                ai2 = ''
            ...
            # can only be used to do at most 8 operations before having to be renewed.
            if (i % 7 == 0) and (i > 0):
                temporary_prompt = self.get_risat_pengabdian_disetujui_pengabdian(data)
            continue
        ...
        while True:
            ...
            output_spreadsheet = filedialog.asksaveasfilename(
                filetypes=[('Excel files', '*.xlsx')],
                initialfile='Sipesat - Disetujui Pengabdian Detil Risat.xlsx',
                title='Simpan sebagai ...'
            )
...
FRAME_CLASSES = (SipesatScrLogin, SipesatScrAbout, SipesatScrLicense, SipesatScrMainMenu, SipesatScrComService, SipesatScrResearch, SipesatScrHarvest,)
